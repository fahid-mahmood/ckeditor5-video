...
export default class VideoStyleEditing extends Plugin {
  ...
  init() {
    ...
    editor.commands.add(
      "videoStyle",
      new VideoStyleCommand(editor, this.normalizedStyles)
    );
  }
  ...
  _setupConversion(isBlockPluginLoaded, isInlinePluginLoaded) {
    ...
    if (isInlinePluginLoaded) {
      ...
      editor.data.upcastDispatcher.on("element:video", viewToModelConverter, {
        priority: "low",
      });
    }
  }
  ...
  _setupPostFixer() {
    ...
    document.registerPostFixer((writer) => {
      ...
      for (const change of document.differ.getChanges()) {
        if (
          change.type === "insert" ||
          (change.type === "attribute" && change.attributeKey === "videoStyle")
        ) {
          let element =
            change.type === "insert"
          ...
          if (
            ...
            !videoStyleDefinition.modelElements.includes(element.name)
          ) {
